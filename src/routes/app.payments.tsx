...
function PaymentsPage() {
  ...
  const cols: Column<Payment>[] = [
    {
      key: "contract",
      header: t("nav.contracts"),
      cell: (r) => contractLabelById.get(r.contractId) ?? `#${r.contractId}`,
    },
    {
      key: "amount",
    ...
    { key: "paid", header: t("common.paidDate"), cell: (r) => formatDate(r.paidDate) },
    {
      ...
      cell: (r) => (
        <StatusBadge tone={paymentStatusTone(r.status)}>
          {t(`paymentStatus.${r.status}`)}
        </StatusBadge>
      ),
    },
  ];
  ...
  const filteredPayments = useMemo(() => {
    ...
    return (list.data ?? []).filter((payment) => {
      const contractMatch = (contractLabelById.get(payment.contractId) ?? "")
        .toLowerCase()
...
function PaymentDialog({
  ...
  const paidDate =
    paidYear || paidMonth || paidDay ? `${paidYear}-${pad2(paidMonth)}-${pad2(paidDay)}` : "";
  return (
    <FormDialog
      key={key}
      open={open}
          ...
          <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center gap-2">
            <Input
              ...
              onChange={(e) => {
                ...
                setDueYear(next);
                if (next.length === 4) dueMonthRef.current?.focus();
              }}
              maxLength={4}
              required
            />
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setDueMonth(next);
                if (next.length === 2) dueDayRef.current?.focus();
              }}
              onBlur={() => setDueMonth((m) => clampMonth(m))}
              onKeyDown={(e) => onSegKey(e, dueMonth, dueYearRef)}
              maxLength={2}
              required
            />
            <span className="text-muted-foreground">/</span>
            <Input
              ...
              onBlur={() => setDueDay((d) => clampDay(d))}
              onKeyDown={(e) => onSegKey(e, dueDay, dueMonthRef)}
              maxLength={2}
              required
            />
          </div>
          ...
          <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center gap-2">
            <Input
              ...
              onChange={(e) => {
                const next = sanitizeDigits(e.target.value, 4);
                setPaidYear(next);
                if (next.length === 4) paidMonthRef.current?.focus();
              }}
              maxLength={4}
            />
            <span className="text-muted-foreground">/</span>
            <Input
              ...
              onChange={(e) => {
                const next = sanitizeDigits(e.target.value, 2);
                setPaidMonth(next);
                if (next.length === 2) paidDayRef.current?.focus();
              }}
              onBlur={() => setPaidMonth((m) => clampMonth(m))}
              onKeyDown={(e) => onSegKey(e, paidMonth, paidYearRef)}
              maxLength={2}
            />
            <span className="text-muted-foreground">/</span>
            <Input
              ...
              onBlur={() => setPaidDay((d) => clampDay(d))}
              onKeyDown={(e) => onSegKey(e, paidDay, paidMonthRef)}
              maxLength={2}
            />
          </div>
        </div>
